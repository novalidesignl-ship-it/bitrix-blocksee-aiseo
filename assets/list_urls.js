(function () {
    ...
    function runOne(idx, btn) {
        ...
        return call('generateAndSave', { id: it.id })
    ...
    function initBulkGenerate() {
        ...
        btn.addEventListener('click', async () => {
            ...
            if (!confirm(`Запустить генерацию описаний для ${indices.length} товаров? Если закроете вкладку, прогресс остановится.`)) {
                return;
            }
            btn.disabled = true;
            ...
            for (const idx of indices) {
                ...
                setRowStatus(idx, 'processing', '');
                try {
                    await call('generateAndSave', { id: it.id });
